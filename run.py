...
@click.option('-i', '--input', 'input_file',
              default='input/p1.png',
              type=click.Path(exists=True, dir_okay=False, readable=True),
              help='Video or picture file name')
@click.option('-o', '--output', 'output_file',
              default=None,
              type=click.Path(exists=False, dir_okay=False, writable=True),
              help='Filename to write results to (video only)')
...
@click.option('--fps', 'output_fps', 
              ...
              help='Output file FPS (0 - same as source)')
...
def main(
    ...
    print(f'Model type: {model_type}\nModel args: {pformat(model._model_args, indent=2, compact=True)}')

    def input_iter(as_image: bool = False) -> Generator[ImageT, None, None]:
        while True:
            if (frame := input.read()) is None:
                break
    ...
    result_frame = None
    stop = False
    for frame in tqdm(input_iter(model.NEEDS_PILLOW), total=input.frame_count):
